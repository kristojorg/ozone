...
export const useCommunicationTemplateList = (options?: {
  ...
  return useQuery({
    enabled: !!labeler && (options?.enabled ?? true),
    ...
    queryKey: ['communicationTemplateList', { for: labeler?.did ?? null }],
    queryFn: async () => {
      const { data } =
        await labeler!.api.tools.ozone.communication.listTemplates()
...
export const useCommunicationTemplateEditor = (templateId?: string) => {
  ...
  const saveFunc = useCallback(
    async ({
      ...
      return templateId
        ? labeler.api.tools.ozone.communication.updateTemplate(
            {
              id: `${templateId}`,
              contentMarkdown,
              subject,
              name,
              disabled,
              updatedBy: labeler.getDid(),
            },
            { encoding: 'application/json' },
          )
        : labeler.api.tools.ozone.communication.createTemplate(
            {
              contentMarkdown,
              subject,
              name,
