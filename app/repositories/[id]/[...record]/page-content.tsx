  ...
  const {
    ...
    queryFn: async () => {
      ...
      const getRecord = async () => {
        const { data: record } =
          await labeler!.api.tools.ozone.moderation.getRecord({ uri })
        return record
      }
      const getThread = async () => {
        ...
        try {
          const { data: thread } =
            await labeler!.api.app.bsky.feed.getPostThread({ uri })
          return thread
        } catch (err) {
      ...
      const getListProfiles = async () => {
        ...
        const { data: listData } = await pds!.api.app.bsky.graph.getList({
          list: uri,
        })
